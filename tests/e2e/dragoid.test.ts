...
describe('Dragoid E2E', async () => {
  ...
  describe('Command preview', async () => {
    ...
    it('Should start reader/server on 0.0.0.0 host using "-p" or "--public" flag', async () => {
      ...
      const shortFlagResponse = await executeCMD(cliPath,
        ['preview', '-f=./tests/assets/novel-example.json', '-p'],
        config
      )

      const longFlagResponse = await executeCMD(cliPath,
  ...
  describe('Command download', async () => {
    ...
    const outputNovelPathTxt = path.resolve(process.cwd(), outputNovelFileNameTxt);
    const outputChapterPathTxt = path.resolve(process.cwd(), outputChapterFileNameTxt);

    const outputNovelFileNameEpub = 'Mocked Novel.epub'
    const outputNovelPathEpub = path.resolve(process.cwd(), outputNovelFileNameEpub);

    afterEach(async () => {
      try {
        await fs.unlink(outputNovelPath).catch(() => {/* ignora se não existe */ });
        await fs.unlink(outputChapterPath).catch(() => {/* ignora se não existe */ });
        await fs.unlink(outputNovelPathTxt).catch(() => {/* ignora se não existe */ });
        await fs.unlink(outputChapterPathTxt).catch(() => {/* ignora se não existe */ });
        await fs.unlink(outputNovelPathEpub).catch(() => {/* ignora se não existe */ });
      } catch (_) {
    ...
    it('Should return error when the command is called without args', async () => {
      await assert.rejects(executeCMD(cliPath, ['download']), (err: Rejected) => {
        assert.deepEqual(err.code, 1);
        assert.match(err.stderr, /Validation error:\s*Missing required option\(s\):\s*['"]--mode['"],\s*['"]--url['"],\s*['"]--format['"]/)
        return true
      })
    })
    ...
    it('Must have mock bot in list of registered bots', async () => {
      ...
      const res = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } });
      assert.match(res.stdout, /List Bots/)
      assert.match(res.stdout, /mock-bot/)
    })

    it('Should display the list of output format supported! Required .json for both modes', async () => {
      const res = await executeCMD(cliPath, ['download', '--list-output-formats']);
      assert.match(res.stdout, /Supported output formats, organized by download type/)
      const isJSon = res.stdout.split("Chapter:").filter(arr => arr.includes('json'))
      assert.deepEqual(isJSon.length, 2, 'Required output format ".json" not found')
    })
    ...
    it('Should download novel with success using mock params and generate file', async () => {
      ...
      const command = ['download', '--url=https://mock-bot.com', '--mode=novel', '--format=json'];
      const response = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } })
      const regex = new RegExp(`File "${safeName}" has been written successfully`);
      assert.match(response.stdout, regex);
    ...
    it('Should download novel with success and generate .txt file', async () => {
      const format = 'txt'
      const safeName = escapeRegex(outputNovelFileNameTxt.replace(`.${format}`, ''))
      const command = ['download', '--url=https://mock-bot.com', '--mode=novel', `--format=${format}`];
      const response = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } })
      const regex = new RegExp(`File "${safeName}" has been written successfully`);
      assert.match(response.stdout, regex);
    ...
    it('Should download novel with success and generate .epub file', async () => {
      const format = 'epub'
      const safeName = escapeRegex(outputNovelFileNameEpub.replace(`.${format}`, ''))
      const command = ['download', '--url=https://mock-bot.com', '--mode=novel', `--format=${format}`];
      const response = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } })
      const regex = new RegExp(`File "${safeName}" has been written successfully`);
      assert.match(response.stdout, regex);
    ...
    it('Should download chapter with success using mock params and generate file', async () => {
      ...
      const command = ['download', '--url=https://mock-bot.com', '--mode=chapter', '--format=json'];
      const response = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } })
      const regex = new RegExp(`File "${safeName}" has been written successfully`);
      assert.match(response.stdout, regex);
    ...
    it('Should download chapter with success and generate .txt file', async () => {
      const format = 'txt'
      const safeName = escapeRegex(outputChapterFileNameTxt.replace(`.${format}`, ''))
      const command = ['download', '--url=https://mock-bot.com', '--mode=chapter', `--format=${format}`];
      const response = await executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } })
      const regex = new RegExp(`File "${safeName}" has been written successfully`);
      assert.match(response.stdout, regex);
    ...
    it('Should download the novel successfully into a custom directory when -p/--path is provided.', async () => {
      ...
      const commands = [
        ['download', '--url=https://mock-bot.com', '--mode=novel', `--format=${format}`, `-p=${tmpdir()}`],
        ['download', '--url=https://mock-bot.com', '--mode=novel', `--format=${format}`, `--path=${tmpdir()}`]
      ]
    ...
    it('Should fail when the custom path passed in -p/--path does not exist', async () => {
      const command = ['download', '--url=https://mock-bot.com', '--mode=novel', `--format=json`, `-p=./invalid-path`];
      const opt = { env: { 'USE_BOT_MOCK': 'true' } };
      await assert.rejects(executeCMD(cliPath, command, opt), (err: Rejected) => {
        assert.deepEqual(err.code, 1)
        assert.match(err.stderr, /Provided path does not exist or is not a directory/)
        return true;
      })
    })
    ...
    it('Should return error if all three required arguments are not provide together', async () => {
      ...
      const commandOpt4 = ['download', '--url=https://mock-bot.com', '--mode=chapter']
      const commandOpt5 = ['download', '--url=https://mock-bot.com', '--format=json']
      const commandOpt6 = ['download', '--format=json', '--mode=chapter']

      const commands = [commandOpt1, commandOpt2, commandOpt3, commandOpt4, commandOpt5, commandOpt6];
    ...
    it('Should return error if skip or limit arguments are used with --mode=chapter', async () => {
      const command = ['download', '--url=https://mock-bot.com', '--mode=chapter', '--format=json', '--limit=2', '--skip=4'];
      await assert.rejects(executeCMD(cliPath, command, { env: { 'USE_BOT_MOCK': 'true' } }), (err: Rejected) => {
        assert.deepEqual(err.code, 1)
        return true
      })
    })
