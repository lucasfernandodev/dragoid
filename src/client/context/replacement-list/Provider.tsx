...
export const ReplacementListProvider = ({
  children,
}: {
  ...
  const [activeList, setActiveList] = useLocalStorage<null | string>(
    'active-replacement-list',
    null
  )

  const getList = (id: string) =>
    collection.find((list) => list.id === id) || null
  ...
  const updateList = (id: string, data: ReplacementList['list']) => {
    setCollection((old) =>
      old.map((list) => (list.id === id ? { id: id, list: data } : list))
    )
  }
  ...
  const getAtiveList = useCallback(
    () => collection.find((list) => list.id === activeList) || null,
