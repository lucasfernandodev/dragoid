...
const paragraphGapOptions = [24, 32, 42, ...lineHeightOptions.map(option => option * 2)]

export const ChapterStyleSelectOptions = {
  fontSize: fontSizeOptions.map(option => ({ label: option, value: option })),
  fontFamily: fontFamilyOptions.map(option => ({ label: option, value: option })),
  lineHeight: lineHeightOptions.map(option => ({ label: option, value: option })),
  paragraphGap: paragraphGapOptions.map(option => ({ label: option, value: option }))
}
