...
import { IconDeviceFloppy, IconDownload, IconEdit, IconPlus, IconTrash, IconUpload, IconX } from '@tabler/icons-react';
...
import { parseFileToJson, type ParseFileJsonError } from '../../../../../utils/parse-file-to-json.ts';
...
export const ListAdder: FC<IListAdderProps> = ({ onOpenCreateList, onUploadList }) => {
    ...
    <div className={S.listAdder}>
      ...
      <Button variant='secondary' className={S.btn} asChild>
        <label htmlFor="upload-list" className={S.button}>
          <IconUpload />
          <span>Upload</span>
          <input accept='.json' onChange={parseUpload} id="upload-list" type="file" hidden />
        </label>
      </Button>
    </div>
  )
...
export const ListCreator: FC<IListCreatorProps> = ({
  ...
  const onSubmit = (ev: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = Object.fromEntries(new FormData(ev.currentTarget)) as { id: unknown }
  ...
  return (
    <form className={S.listCreator} onSubmit={onSubmit}>
      <div className={S.form_group}>
        ...
        <input data-invalid={!!error} name="id" id="list-id" type="text" className={S.form_input} placeholder='List identify...' />
...
export const ListItem: FC<IListItemProps> = ({ id, activeListId, actions, ...props }) => {
...
export const RenderList: FC<IRenderListProps> = ({ lastCreateId, onEditListActive }) => {
  const { getCollection, getList, getActiveList, deleteList, toggleListActive } = useReplacementList()
  ...
  const actions = ({ id, list }: ReplacementList) => ({
    deleteList: () => {
      setListIds(oldLists => ([...oldLists].filter(
...
export const GlobalError: FC<IGlobalErrorProps> = ({ error, onCleanError }) => {
  ...
  return (
    <div className={S.global_error}>
      ...
      <Button className={S.btn} onClick={onCleanError} variant='secondary'><IconX /></Button>
    </div>
  )
}
...
export const TextReplacementOverview: FC<ITextReplacementOverviewProps> = ({
  ...
  return (
    <div className={S.overview}>
      <ListAdder onOpenCreateList={openCreateList} onUploadList={onUploadHandle} />
      <GlobalError onCleanError={() => setGlobalError(null)} error={globalError} />
      <RenderList onEditListActive={onEditListActive} lastCreateId={updatedLastId} />
    </div>
  )
}