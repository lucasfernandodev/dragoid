...
export function Select<T extends string | number>({ values, placeholder, defaultValue, onChange, className}: SelectProps<T>){
  ...
  useEffect(() => {
    ...
    // Overflow not detect
    if (!isOverflowBottom && !isOverflowTop) {
      console.log('select open to bottom')
      list.style.top = `${top + height}px`
    }
    // overflow bottom
    if (isOverflowBottom && !isOverflowTop) {
      console.log('select open to top')
      list.style.top = `${(top) - listHeight}px`;
    }
    // overflow top
    if (!isOverflowBottom && isOverflowTop) {
      ...
      if (isFocus && showDropdown) {
        ...
        switch (e.key) {
          ...
          case 'ArrowUp':
            ...
            const prevElement = itemFocused.previousElementSibling as HTMLElement | null;
            if (prevElement) {
              prevElement.focus()
          ...
          case 'ArrowDown':
            ...
            const nextElement = itemFocused.nextElementSibling as HTMLElement | null;
            if (nextElement) {
              nextElement.focus()
          ...
          case ' ': // Space
            ...
            if (itemFocused) {
              ...
              if (!value) return;
              const isOptions = values.filter(option => option.value === value);
  ...
  useEffect(() => {
    const isDefault = values.find(
      option => option.label === defaultValue?.label && option.value === defaultValue?.value
    )
  ...
  return (
    <div className={S.container_select} ref={containerRef} data-expanded={showDropdown}>
      <input
        type="text"
        ...
        {values.map(option => {
          ...
          return (
            <li
              ...
              data-active={defaultValue?.value === value || selectedValue?.value === value}
              aria-selected={defaultValue?.value === value || selectedValue?.value === value}
              role="option"
              onFocus={() => setIsFocus(true)}
              tabIndex={-1}
