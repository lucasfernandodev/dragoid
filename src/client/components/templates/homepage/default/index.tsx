...
export const HomepageDefaultTemplate: FC<HomepageDefaultTemplateProps> = ({
  ...
  return (
    <App className={S.app} appTitle={`Dragoid | ${book.title}`} ref={chapterRef}>
      <section className={S.app_thumbnail}>
        ...
        <p className={S.author}>{book.author.map(a => (<span key={a}>{a}</span>))}</p>
        <ul className={S.book_details}>
          <li><h3>Status</h3><span>{book.status}</span></li>
          <li><h3>Chapter</h3><span>{book.qtdChapters}</span></li>
          <li><h3>Language</h3><span>{book.language}</span></li>
        </ul>
        <div className={S.sinopse}>
          {book.sinopse.map((t, index) => (<p key={index}>{t}</p>))}
        </div>
