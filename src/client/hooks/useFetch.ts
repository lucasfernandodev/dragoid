...
const fetchReducer = <T>(state: FetchState<T>, action: FetchAction<T>): FetchState<T> => {
  ...
  switch (action.type) {
    case "FETCH_START":
      ...
      return { isLoading: true, isFetching: true, data: state.data, success: false }
    case "FETCH_SUCCESS":
      if (state.data !== null && JSON.stringify(state.data) === JSON.stringify(action.payload)) {
        return { data: state.data, isLoading: false, success: true, isFetching: false }
      }
      return { data: action.payload, isLoading: false, success: true, isFetching: false }
    case "FETCH_ERROR":
      return { isLoading: false, data: null, success: false, errorMessage: action.message, isFetching: false }
