...
function hasArgv(obj: unknown): obj is { argv: CLIArgs } {
  return !!obj && typeof obj === 'object' && 'argv' in obj
}

export const yargsFailHandle = (
  message: unknown,
  error: Error,
  yargs: Argv<CLIArgs>
) => {
  ...
  const yargsParsed = yargs.parsed
  const _argv = hasArgv(yargsParsed) ? yargsParsed.argv : undefined
