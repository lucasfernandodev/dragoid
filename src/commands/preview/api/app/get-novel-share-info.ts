  ...
  app.get("/api/share", async (_, reply) => {
    ...
    if (!app.isPublic) {
      return reply.send({
        ...
        data: {
          isPublic: false,
          message: 'The reader is only available on this device. To enable access on other devices on the local network, start the server with the --public (or -p) option.'
        }
      })
    }
